...
public class Priority {
	...
	public static void main(String[] args) {
		...
		String appoggiod="";
		int appoggio2=0;
		int ultimo=tempoDiEsecuzione.length-1;
		int appoggio=0;
		int appoggioP = 0;
		for(superficie=primo;superficie<=ultimo-1;superficie=superficie+1){
			  for(int posizione=ultimo-1;posizione>=superficie;posizione--){
			    if(tempoDiPriorita[posizione]<tempoDiPriorita[posizione+1]){
			    	
			     appoggioP=tempoDiPriorita[posizione];
			     tempoDiPriorita[posizione]=tempoDiPriorita[posizione+1];
			     tempoDiPriorita[posizione+1]=appoggioP;
		...
		for(int i=0; i<tempoDiArrivo.length;i++) {
			tempoDiAttesa[i] = tempoDiServizio[i] - tempoDiArrivo[i];
				if(tempoDiAttesa[i]<0) {
					tempoDiAttesa[i] = 0;
				}
			media += tempoDiAttesa[i];
		}
		System.out.println("attesa: "+Arrays.toString(tempoDiAttesa));
		System.out.println("il tempo d'attesa medio è: "+ media/4);
		//output
		for(int i=0; i<tempoDiArrivo.length;i++) {
